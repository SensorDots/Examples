...
{
    if (i2c_start_external(address | I2C_WRITE)) return 1;

    for (uint16_t i = 0; i < length; i++)
    {
        if (i2c_write_external(data[i])) return 1;
    }
...
{
    if (i2c_start_external(address | I2C_READ)) return 1;

    for (uint16_t i = 0; i < (length-1); i++)
    {
        data[i] = i2c_read_ack_external();
    }

    data[(length-1)] = i2c_read_nack_external();
    i2c_stop_external();
    return 0;
}
...
{
    if (i2c_start_external(devaddr | 0x00)) return 1;

    i2c_write_external(regaddr);
...
{
    if (i2c_start_external(devaddr)) return 1;

    i2c_write_external(regaddr);

    if (i2c_start_external(devaddr | 0x01)) return 1;
